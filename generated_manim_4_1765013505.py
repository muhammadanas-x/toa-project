...
class ExponentialEvenDemo(Scene):
    def construct(self):
        ...
        graph_f_negx_label = axes.get_graph_label(graph_f_negx, label='f(-x) = e^(-4x)', x_val=2)
        self.play(Create(graph_f_negx), Write(graph_f_negx_label))
        self.wait(1)

        graph_neg_fx = axes.plot(neg_fx, color=GREEN, x_range=[-3, 3])
        graph_neg_fx_label = axes.get_graph_label(graph_neg_fx, label='-f(x) = -e^(4x)', x_val=-2)
        self.play(Create(graph_neg_fx), Write(graph_neg_fx_label))
        self.wait(1)

        explanation = Text("e^(4x) is neither even nor odd.").scale(0.7)
