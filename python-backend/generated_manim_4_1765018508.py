...
class OddEvenSqrtDemo(Scene):
    def construct(self):
        axes = Axes(x_range=[-4, 4, 1], y_range=[-10, 10, 2], x_length=7, y_length=5, tips=True)
        ...
        graph_fx_label = axes.get_graph_label(graph_fx, label='f(x) = (x+1)² (neither even nor odd)')
        self.play(Create(graph_fx), Write(graph_fx_label))
        self.wait(1)

        graph_f_negx = axes.plot(f_negx, color=RED, x_range=[-4, 4])
        graph_f_negx_label = axes.get_graph_label(graph_f_negx, label='f(-x)', x_val=3)
        self.play(Create(graph_f_negx), Write(graph_f_negx_label))
        self.wait(1)

        graph_neg_fx = axes.plot(neg_fx, color=GREEN, x_range=[-4, 4])
        graph_neg_fx_label = axes.get_graph_label(graph_neg_fx, label='-f(x)', x_val=-3)
